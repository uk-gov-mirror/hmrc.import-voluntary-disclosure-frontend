...
class UploadAnotherFileViewSpec extends ViewBaseSpec with BaseMessages {
  ...
  "Rendering the UploadAnotherFile page" when {
    "no errors exist when one file is present" should {
      ...
      lazy val view: Html = injectedView(form, Call("GET", controllers.routes.UploadAnotherFileController.onLoad().url),answers)(fakeRequest, messages)
    ...
    "no errors exist when two files are present" should {
      ...
      val form: Form[Boolean] = formProvider.apply()
      lazy val view: Html = injectedView(form, Call("GET", controllers.routes.UploadAnotherFileController.onLoad().url),answersTwoFiles)(fakeRequest, messages)
      lazy implicit val document: Document = Jsoup.parse(view.body)
